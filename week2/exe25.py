...
def main():
    ...
    with Pool(5) as p:
        crawled_lematizedtexts = list(p.map(process_text, nltktexts))
    print()
    ...
    "Prune the vocabulary for the exercise 2.5 of week 3"
    print("prune vocabulary")
    pruning_decisions = prune_text(unifiedvocabulary, highest_totaloccurrences_indices, unifiedvocabulary_totaloccurrencecounts)

    #%% Get indices of documents to remaining words
    oldtopruned=[]
    tempind=-1
    for k in range(len(unifiedvocabulary)):
        if pruning_decisions[k]==0:
            tempind=tempind+1
    ...
    for k in range(len(crawled_lematizedtexts)):
        ...
        temp_newindices=[]
        temp_newdoc=[]
        for l in range(len(crawled_lematizedtexts[k])):
    ...
    remainingindices=np.squeeze(np.where(pruning_decisions==0)[0])
